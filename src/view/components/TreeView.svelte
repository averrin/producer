<script>
   ...
   function itemClick(e) {
      ...
      const isFolder = node.source instanceof Folder;
      if (!isFolder) {
      ...
      for (const ch of newItems) {
         if (ch.id in nodes && nodes[ch.id].source) {
            ...
            if (doc instanceof Folder) {
               u = dir_updates;
               changes["sort"] = dn;
               changes["parent"] = node.source.id || null;
               dn++;
            } else {
               u = updates;
               changes["sort"] = n;
               changes["folder"] = node.source.id || null;
               n++;
            }
   ...
   function handleDndFinalize(e) {
      ...
      if (sel && $selected.length) {
         ...
         if (node.id != "root") node.source?.update({ sorting: "m" });
